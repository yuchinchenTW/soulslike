# 灰燼之庭 · ASHFALL

原創 3D 第三人稱魂系戰鬥原型。使用 Three.js。場景、介面與音效為專案自製，角色與動畫來自 Adobe Mixamo（見 `CREDITS.md`），沒有使用《黑暗靈魂》的素材。

**線上試玩：** https://soulslike-96jd.onrender.com/

![遊戲畫面](demo.png)

## 啟動

Windows 雙擊 **start-game.cmd**，保留啟動的命令列視窗；若瀏覽器先開啟而顯示無法連線，稍等一秒後重新整理。

也可在此資料夾執行：

```sh
npm install --ignore-scripts
npm start
```

開啟 http://127.0.0.1:4173。需要 Node.js，以及支援 WebGL 2 的桌面 Chrome / Edge。套件安裝完成後，遊戲不需網路連線。不要直接雙擊 index.html，ES 模組需要本機伺服器。

## 操作

| 操作 | 按鍵 |
| --- | --- |
| 移動 | WASD |
| 視角 | 滑鼠；方向鍵左右亦可旋轉 |
| 調整鏡頭距離 | 未鎖定時滾動滑鼠滾輪 |
| 切換鎖定敵人 | 已鎖定時，滾輪向下下一個、向上上一個 |
| 三段輕攻擊 | 連按滑鼠左鍵 / J，在收招時輸入下一擊 |
| 重攻擊 | R |
| 持盾防禦 | 按住滑鼠右鍵 / K |
| 盾反 | 短按並放開滑鼠右鍵 / K（180 毫秒內）揮盾；持續按住只防禦。成功後以輕攻擊處決 |
| 翻滾 | Space，搭配方向鍵 WASD |
| 奔跑 | 按住 Shift |
| 鎖定 / 解除 | 按下滑鼠中鍵（滾輪） / Q |
| 補血 | F |
| 篝火休息 / 拾回餘燼 | E |
| 暫停 | Esc |
| 操作說明 | H |
| 音效開關 | M |

## 本版內容

- 可探索的廢棄庭院、教堂立面、石柱碰撞、環繞鏡頭、篝火、灰燼粒子與動態陰影。石材採 PBR 貼圖與黃昏 HDRI 天空。
- 劍盾角色、攻擊前搖與後搖、翻滾無敵時間、正面格擋、體力恢復與防禦崩潰。防禦鍵短按放開後揮盾：動作開始後 0.1–0.36 秒為盾反判定窗，成功時敵人（含頭目與分身）失衡，可用輕攻擊處決。持續按住立即防禦，不先揮盾；已格擋攻擊的短按放開也不會追加盾反。盾反完整收盾後才能接回防禦。
- 兩名守衛與「雙誓教長」頭目。火焰與魔力雙劍、三／五連擊、蓄力突進，半血後召喚可擊殺的分身。
- 三次餘火瓶補給；篝火回復生命與補給、重置普通敵人。篝火與火盆為程序化火焰與火星。
- 死亡掉落餘燼、篝火重生及掉落物回收。再次死亡會取代先前掉落。
- 擊敗頭目可繼續探索或重新挑戰；暫停、操作指南與合成音效。

這是單一關卡的可玩原型，約 5–10 分鐘，場景使用幾何造型美術。尚未包含完整 RPG、裝備成長、存檔、手把或手機操作。

## 程式結構與檢查

- `src/game.js`：與渲染分離的戰鬥及重生邏輯。
- `src/world.js`：Three.js 場景、角色動畫及鏡頭。
- `src/main.js`：輸入、HUD、介面與音效。
- `server.mjs`：僅監聽本機的靜態伺服器。

```sh
npm run check
npm test
```

Three.js 採 MIT 授權，授權內容見 `node_modules/three/LICENSE`。

## v0.3 更新

玩家、頭目與守衛改用匯入的貼圖蒙皮模型與骨骼動畫，取代先前的幾何造型角色。共 13 段動作剪輯（待機、行走、奔跑、前後左右移動、輕攻擊、重攻擊、格擋、翻滾、受擊、補血、死亡），由 `tools/build-character-clips.mjs` 從 FBX 重新產生為 `assets/characters/*-clips.json`。

角色與動畫素材來自 Adobe Mixamo，授權與來源說明見 `CREDITS.md`。場景、介面、玩法與合成音效仍為專案自製。

## v0.4 更新

- 玩家輕攻擊依序為斜斬、反向斬、蓄力終結，使用三段不同的骨骼動畫。耗體力分別為 23／22／28，傷害為 30／30／42；停止連段 0.65 秒或使用翻滾、重攻擊、受擊後會重置。
- 收招前 0.38 秒可預輸入下一次攻擊或翻滾，保留完整後搖。介面顯示目前連段。
- 「雙誓教長」以《黑暗靈魂 III》教宗沙力萬的雙劍與影分身戰鬥為靈感：右手火劍、左手魔劍，使用匯入的雙持連擊動畫，另有金色冠飾與隨骨骼運動的開衩長袍。
- 頭目有三連擊、五連擊與突進斬，快慢交錯，收招後有 1.15–1.4 秒反擊時間。出劍前停止追蹤玩家方向。
- 半血時蓄力震波並召喚 100 生命的半透明分身；分身先出招，本體晚 0.55 秒跟進。分身可鎖定、擊殺，不提供頭目獎勵；消滅後頭目需等待召喚冷卻。死亡、休息與擊敗本體都會清除分身。
- 角色仍以現有 Mixamo 模型改造，並非教宗原版模型或原版動作的完整重製。

`src/motion.js` 定義玩家動作與命中時機，`src/pontiff.js` 定義頭目招式時間軸與分身協同，`src/regalia.js` 建立頭目的骨骼長袍及冠飾。動畫來源與新增三段 FBX 記錄於 `CREDITS.md` 和 `assets/source-manifest.json`。

除 `npm run check` 與 `npm test` 外，`tests/browser-smoke.cjs` 驗證遊戲流程，`tests/character-review.cjs` 驗證模型及翻滾接地，`tests/boss-review.cjs` 驗證三段斬擊、雙劍接觸、第二階段與分身清除。瀏覽器檢查需要本機伺服器、Chrome 與 Puppeteer（可用 `PUPPETEER_PATH` 指定既有安裝位置）。

## v0.4.1 雙劍與效能調整

教長與分身的劍身從護手往外加長 85%、加寬 50%，握柄尺寸保持貼合手掌；劍光端點與攻擊距離一併調整。

固定建築依材質及空間區塊合併繪製；角色接地改用預存取樣點及共用骨骼矩陣，保留原有取樣精度。分身模型與材質在載入時預先準備，移除雙劍的額外點光源，以自發光材質呈現亮劍。分身採單次透明繪製，消失後的敵人停止骨骼更新。陰影貼圖調整為 1536，畫布限制原生 1 倍像素密度、最高約 1080p，降低高解析度螢幕負擔。

本機 Chrome 無介面測試、1440×900、相同第二階段流程取樣 180 幀：畫面繪製次數 165 → 86；CPU 更新及提交繪製時間中位數約 2.2 → 0.9 ms。這是本機比較，並非所有裝置的 FPS 保證。比較結果存放於 `artifacts/performance-before.json` 與 `artifacts/performance-after.json`。

## v0.4.2 FPS 與自動畫質

右上角常駐顯示 FPS、平均每幀毫秒數及版本號，每約半秒更新一次。讀值使用真正的瀏覽器畫面更新間隔，包含可見的長幀，不使用戰鬥邏輯的 50 ms 上限；切回分頁會重新取樣。

畫質預設為「自動」：持續低於 50 FPS 時逐步降低畫布解析度，穩定高於 58 FPS 八秒後才慢慢恢復。按 Esc 暫停即可使用右上角選單選「流暢」（較低解析度、關閉動態陰影）或「精緻」（固定較高解析度）。文字介面維持清晰。陰影貼圖降至 1024、更新頻率最高每秒 30 次，並移除全畫面顆粒濾鏡及不必要的介面文字重寫。

雙劍劍身改為原始素材的 2.8 倍長、2.2 倍寬（相較 v0.4.1 再長約 51%、再寬約 47%）；握柄維持原尺寸，劍光和攻擊距離同步增加。

`tests/performance.test.js` 驗證 FPS 計算、長幀、分頁重置與自動解析度調整；`tests/fps-review.cjs` 實測 FPS 介面、畫質切換、高 DPI 解析度上限及介面位置。

## v0.5 場地質感

牆面、石柱、拱門、地板與基座改用 Poly Haven 的 CC0 PBR 貼圖（漫反射、法線、AO／粗糙度），以世界座標三平面投影取樣，因此以縮放方塊搭建的建築不需重做 UV，靜態合批也不受影響。地面由實例化石磚改為單一石板平面。

環境光與反射改用黃昏 HDRI 天空，霧色與地平線一致；半球光與補光減弱，讓太陽方向的陰影更明顯。新增素材約 7MB，皆為 1K 解析度。來源記錄於 `CREDITS.md` 與 `assets/source-manifest.json`。

篝火與火盆改為程序化火焰：面向鏡頭的噴焰噪聲面片、GPU 驅動的上升火星、煙與光暈，全部由單一時間 uniform 驅動，不佔 CPU。四座火盆新增閃爍點光，「流暢」模式會關閉。實作在 `src/fire.js`。

篝火中的劍改用聖騎士模型自帶的貼圖長劍，刃朝下插在餘火中。地面的草由圓錐體改為交叉的葉片卡片：程序繪製的乾草貼圖、逐叢色調變化與頂端隨風擺動，集中在拱廊與牆基。實作在 `src/grass.js`，劍由 `createSwordProp` 產生。

碎石、篝火石圈與草叢改用 Poly Haven 的 CC0 掃描模型（`rock_09`、`rock_07`、`stone_fire_pit`、`grass_medium_02`），以實例化方式擺放，草叢沿用風搖著色器。火盆改為車削成形的鐵碗與發光炭塊。模型載入與擺放在 `src/props.js`。

## v0.6.1 盾反修正

右鍵／K 短按放開觸發一次盾反，長按持續防禦。揮盾與軀幹姿勢烘焙成獨立動畫，平順銜接起手、收盾與受擊；固定姿勢不再累積骨骼旋轉。盾反成功與重新按住防禦都不會截斷收盾。

`tests/guard-input.test.js` 檢查短按、長按、格擋與中斷輸入；`tests/parry-review.cjs` 以瀏覽器檢查右鍵／K 操作、連續盾反、不同朝向、移動銜接與受擊過渡，並輸出 `artifacts/parry-review.json`。

## v0.6.2 揮盾與敵人失衡

盾反揮盾角度加大 80%。成功後守衛、頭目與分身使用獨立失衡動畫：快速彎身、武器被撥開，維持破綻姿勢後再起身；腳下金色光圈與成功提示標示可處決狀態。敵人的連擊立即停止，失衡時間仍為守衛 2.2 秒、頭目與分身 1.4 秒。

`tests/parry-reaction-review.cjs` 驗證實際敵人攻擊被盾反後的動畫、失衡姿勢維持與提示光圈。

## v0.6.3 盾反全身動作

揮盾加入重心轉移、髖部與軀幹帶動，以及肩、肘、手腕依序跟隨和緩慢收回。雙腳以腿部 IK 維持踩地。敵人失衡改為連續播放受擊步伐，搭配武器擺動、身體晃動與小幅退步，再逐漸恢復；不再停在單格姿勢。動作生成集中於 `src/shield-motion.js`，僅在載入時烘焙。

瀏覽器檢查包含全身關節活動、踩地滑動、失衡期間持續移動，以及受擊與恢復銜接。

## 部署到 Render


本專案是純靜態網站，可用 Render Static Site 免費部署。`render.yaml` 已包含設定：

- Build Command：`npm install --ignore-scripts --no-audit --no-fund`
- Publish Directory：`.`

在 Render 選「New → Blueprint」連上這個 GitHub repo 即可自動建立服務。
